...
서브쿼리는 `UPDATE`, `DELETE` 문과 `INSERT` 문에서도 많이 사용되며, 상관 서브쿼리는 `UPDATE` 문과 `DELETE` 문에서 자주 쓰인다

다음은 어떠한 목적에 의해서 `last_update` 가 모든 고객에 표시되어야 한다고 가정하자.
...
이는 `customer` 의 모든 테이블에 마지막으로 `rental` 한  시각을 업데이트한다.

`MAX(r.rental_date)` 는 `rental` 한 가장 마지막 시간이므로, `customer` 의 `last_update` 역시 같은 시간에 업데이트 된다.

하지만, 이는 효율적이지 못하다.
`last_update` 는 `NULL` 을 포함하는데, `NULL` 이 있다면 한번도 대여하지 않은 고객이다.
이러한 고객역시 덮어씌워버려, 대여하지 않은 고객을 구분하지 못한다.

다음은 상관 서브쿼리와 함께 `WHERE` 절을 사용하는 `UPDATE` 문의 다른 예제이다
...
이는 단 한번이라도 `rental` 한 `customer` 만 업데이트한다.
즉, 대여기록이 없다면, 여전히 `NULL` 이며, 이를 구분하여 대여한 고객 및 대여하지 않은 고객을 구분할수 있다.

다음은 `DELETE` 를 사용할때의 예이다
