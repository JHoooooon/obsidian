...
[[데이터베이스 생성 및 자료형]]에서 문자열 데이터의 대한 개요를 볼 수 있다.
...
text_fld                                          |
--------------------------------------------------+
This string is 28 characters, but now it is longer|
```

이렇게 `기존의 field` 의 값과 합칠수 있다.
다음은 `first_name` 과 `last_name` 및 여러 문자열을 `concat` 함수로 합치는 예시이다
...
cust_narrative                                      |
----------------------------------------------------+
MARY SMITH has been a customer since2006-02-14      |
PATRICIA JOHNSON has been a customer since2006-02-14|
LINDA WILLIAMS has been a customer since2006-02-14  |
BARBARA JONES has been a customer since2006-02-14   |
ELIZABETH BROWN has been a customer since2006-02-14 |
```
...
#### INSERT(str, start_idx, replace_str_idx, replace_str): 문자열 추가 및 대체 함수

`insert` 는 `str` 을 받고, `start_idx` 의 숫자부터, `replace_str_idx` 까지의 숫자값까지, `replace_str` 로 변경하는 함수이다.

`replace_str_idx` 값이 `0` 이면, 교체되지 않고, `start_idx` 부분에 `replace_str` 을 추가한다.

다음은 `insert` 를 사용하여 문자열 삽입 및 교체를 하는 예시이다
...
`replace_str_count` 가 `0` 임을 볼수 있다.
이는 교체할 문자의 갯수가 없다는것이다.

다음은 대체 문자열의 갯수를 입력해본다
...
`replacE_str_count` 에 입력한 `7` 자리의 문자열인 `goodbye` 가 삭제대고, `hello` 로 대체되는것을 볼 수 있다.

#### REPLACE(origin_str, str, repalce_str): 문자열을 대체하는 함수

`replace` 는 `origin_str` 중 `str` 의 내용을 `replace_str` 로 변경한다

>[!warning] `replace` 함수는 모든 일치되는 `str` 을 `replace_str` 로 변환한다는점을 주의해야 한다.
...
#### SUBSTRING(str, start_idx, end_idx): 부분 문자열을 추출

`substring` 은 `str` 에서 `start_idx` 부터 `end_idx` 의 길이까지 문자열을 자른다
